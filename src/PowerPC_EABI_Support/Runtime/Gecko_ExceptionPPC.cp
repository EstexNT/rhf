...
/*
 * TODO: Pragma hack; fixes instruction swap for __register_fragment
 *       Note that prefixing the function definition with the following fake
 *       function fixes the instruction swap:
 *           void fake_func(ProcessInfo *f) {
 *               f->active = 0;
 *           }
 */
#pragma push
...
int __register_fragment(struct __eti_init_info* info, char* TOC) {
    ...
    for (f = fragmentinfo, i = 0; i < MAXFRAGMENTS; i++, f++) {
        if (f->active == 0) {
    ...
    for (;;) {
        if (iter->info.action_pointer == 0 ||
            ...
            case EXACTION_SPECIFICATION:
                iter->info.action_pointer += sizeof(ex_specification) +
...
/*
 * Allow access to __exception_magic in try/catch blocks
 */
#pragma push
